...
;
ALTER TABLE `geodata`.`_cities` 
ADD INDEX `fk_countryid_cities_idx` (`country_id` ASC) VISIBLE;
;
ALTER TABLE `geodata`.`_cities` 
ADD CONSTRAINT `fk_countryid_cities`
  FOREIGN KEY (`country_id`)
  REFERENCES `geodata`.`_countries` (`id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
ALTER TABLE `geodata`.`_cities` 
ADD INDEX `fk_regionid_citites_idx` (`region_id` ASC) VISIBLE;
;
ALTER TABLE `geodata`.`_cities` 
ADD CONSTRAINT `fk_regionid_citites`
  FOREIGN KEY (`region_id`)
  REFERENCES `geodata`.`_regions` (`id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
ALTER TABLE `geodata`.`_cities` 
ADD INDEX `title2` (`title` ASC) VISIBLE;
;
